    ...
    static
    {
        ...
        COVERAGE_MIN = b
                .comment("Минимальная доля лучей, упирающихся в берег, чтобы чанк был принят (0.0 – 1.0).",
                         "0.7 (старый дефолт) был слишком жёстким — на тестовых seed'ах из 1089 кандидатов",
                         "не находилось ни одного accepted (доля open-ocean чанков с 70% берегом во вне круге мала).",
                         "0.3 даёт хорошую плотность accepted-слотов на любом seed.")
                .defineInRange("coverage_min", 0.30, 0.0, 1.0);
